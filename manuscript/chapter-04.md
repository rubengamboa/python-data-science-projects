...
But classes have a considerable advantage in many cases. In Python, each module exists only once. If you import a module multiple times, you still have only one version of the module, so if the module defines a particular global variable, there is only one version of this global variable, and it is shared wherever the module is imported. However, classes can have many instances, and each instance can have its own copy of its attributes. For example, each `Turtle` has its own values of `x`, `y`, and `dir`. Similarly, you can have multiple instances of the bigram code, one instance for English and the other for German. That is precisely the point of the project in this chapter! 
